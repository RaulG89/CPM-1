...
public class VentanaConfirmacion extends JDialog {
	 ...
	 * @param vR
	 */
	public VentanaConfirmacion(VentanaRegistro ventana) {
		this.vr = ventana;
	...
	private JLabel getLblCheck() {
		if (lblCheck == null) {
			...
			lblCheck.setIcon(new ImageIcon(VentanaConfirmacion.class
		...
		if (btnFinalizar == null) {
			...
			btnFinalizar.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					getVr().getVp().inicializar(); // Limpia el catalogo
					getVr().dispose(); // limpia los datos en ventana registro
					dispose(); // limpia los datos en ventana confirmacion	
				}
			});
			vr.getVp().getPedido().grabarPedido(txtCodigo.getText(), vr);
