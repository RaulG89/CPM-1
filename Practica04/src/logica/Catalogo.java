	...
	private void cargarArticulos() {
		...
		try {
			...
			while (fichero.ready()) { // Mientras queden lineas por leer
				...
				datosArticulo = linea.split("@"); // Corta la linea por el caracter indicado -> crea varias lineas en un array
				articulosCatalogo.add(new Articulo(datosArticulo[0], datosArticulo[1],Float.parseFloat(datosArticulo[2]), 0)); 
				//Añade al ArrayList un nuevo Articulo con los campos del array || Float.parseFloat -> convierte String a float
			}
			fichero.close();
