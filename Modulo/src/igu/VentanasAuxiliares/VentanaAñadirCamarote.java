...
public class VentanaAñadirCamarote extends JDialog {
	...
	private JButton getBtnAñadir() {
		if (btnAñadir == null) {
			...
			btnAñadir
					.setToolTipText("A\u00F1ade al camarote a la lista de camarote de la reserva");
			...
			btnAñadir.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					for (int i = 0; i < arrayCamarotes.size(); i++) {
						if (arrayCamarotes.get(i).isTipo() == tipo
								...
								if (tablaPasajeros.getValueAt(j, 0).equals(
										...
										&& tablaPasajeros.getValueAt(j, 3)
												.equals(pasajerosCamarote
														.get(j)
														.getFechaNacimiento())) {
									arrayCamarotes.get(i).getPasajeros()[plazas] = pasajerosCamarote
											.get(j);
									plazas--;
								}
							}
							if (chckbxCamaSupletoria.isSelected())
								arrayCamarotes.get(i).getPasajeros()[plazas] = (Pasajero) cbPasajerosMenoresEdad
										.getSelectedItem();
							...
							for (int x = 0; x < arrayCamarotes.get(i)
								...
								if (arrayCamarotes.get(i).getPasajeros()[x] != null) {
							...
							vp.getModeloTablaCamarotes().addRow(
									new Object[] { tipo2,
					...
					for (int i = 0; i < tablaPasajeros.getRowCount(); i++) {
						for (int j = 0; j < pasajerosCamarote.size(); j++) {
							if (tablaPasajeros.getValueAt(i, 0).equals(
									pasajerosCamarote.get(j).getNombre())
									&& tablaPasajeros.getValueAt(i, 1).equals(
											pasajerosCamarote.get(j)
													.getApellidos())
									&& tablaPasajeros.getValueAt(i, 2).equals(
											pasajerosCamarote.get(j).getDni())
									&& tablaPasajeros.getValueAt(i, 3).equals(
											pasajerosCamarote.get(j)
													.getFechaNacimiento())) {
								modeloComboPasajeros
										.removeElement(pasajerosCamarote.get(j));
					...
					if (chckbxCamaSupletoria.isSelected()) {
						pasajerosCamarote.remove(cbPasajerosMenoresEdad
								.getSelectedItem());
						vp.getListaClientes().remove(
								cbPasajerosMenoresEdad.getSelectedItem());
						modeloComboMenores.removeElement(cbPasajerosMenoresEdad
								.getSelectedItem());
					}
					...
					if (!valido) {
						...
						for (int i = 0; i < pnRestoExtras.getComponentCount(); i++) {
	...
	private JButton getBtnCancelar() {
		if (btnCancelar == null) {
			...
			btnCancelar
					.setToolTipText("Cierra la ventana actual y vuelve a la ventana principal");
	...
	private JComboBox<String> getCbSituacion() {
		if (cbSituacion == null) {
			...
			cbSituacion
					.setToolTipText("Selecciona la situaci\u00F3n del camarote respecto al barco");
	...
	private JPanel getPnExtras() {
		if (pnExtras == null) {
			pnExtras = new JPanel();
			pnExtras.setBorder(new TitledBorder(null, "Extras:",
				...
				public void actionPerformed(ActionEvent arg0) {
					if (chckbxCamaSupletoria.isSelected()) {
						Pasajero pasajero = (Pasajero) cbPasajerosMenoresEdad
								.getSelectedItem();
						for (int i = 0; i < modeloComboPasajeros.getSize(); i++) {
							if (modeloComboPasajeros.getElementAt(i)
									.getNombre().equals(pasajero.getNombre())
						...
						for (int i = 0; i < vp.getListaClientes().size(); i++)
							modeloComboPasajeros.addElement(vp
	...
	private JComboBox<Pasajero> getCbPasajeros() {
		if (cbPasajeros == null) {
			...
			cbPasajeros
					.setToolTipText("Seleccione al pasajero que va a utilizar el camarote");
	...
	private JButton getBtnAgregarPasajero() {
		if (btnAgregarPasajero == null) {
			...
			btnAgregarPasajero
					.setToolTipText("Agrega al pasajero seleccionado al camarote");
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					if (plazas != pasajeroCamarote) {
						Pasajero pasajero = (Pasajero) cbPasajeros
								.getSelectedItem();
						if (pasajero.isMenor()) {
							for (int i = 0; i < modeloComboMenores.getSize(); i++) {
						...
						for (int i = 0; i < modeloTablaPasajeros.getRowCount(); i++)
							if (modeloTablaPasajeros.getValueAt(i, 0).equals(
									pasajero.getNombre())
	...
	private JButton getBtnBorrarPasajero() {
		if (btnBorrarPasajero == null) {
			...
			btnBorrarPasajero
					.setToolTipText("Borra al pasajero seleccionado del camarote");
	...
	private void añadirExtras() {
		...
		for (Extra a : vp.getCrucero().getBarco().getCamarotesBarco().get(0)
			...
			if (!a.getDescripcion().equals("Cama supletoria")) {
				...
				JCheckBox aux = new JCheckBox(a.getDescripcion() + " ("
						+ a.getPrecio() + " €)");
				aux.setMnemonic(e);
				pnRestoExtras.add(aux);
			} else {
				cama = true;
				chckbxCamaSupletoria.setText("Cama supletoria ("
						+ a.getPrecio() + " €)");
			}
		}
		if (menor && cama) {
	...
	private void comprobarModelos() {
		...
		if (cbTipo.getSelectedItem().equals("Familiar") && familiarE == 0)
			modeloComboPosicion.removeElement("Exterior");
		else if (!cbTipo.getSelectedItem().equals("Familiar") && familiarE != 0) {
			añadido = false;
			for (int i = 0; i < modeloComboTipo.getSize(); i++) {
		...
		if (cbTipo.getSelectedItem().equals("Familiar") && familiarI == 0)
			modeloComboPosicion.removeElement("Interior");
		else if (!cbTipo.getSelectedItem().equals("Familiar") && familiarI != 0) {
			añadido = false;
			for (int i = 0; i < modeloComboTipo.getSize(); i++) {
				if (modeloComboTipo.getElementAt(i).equals("Familiar"))
					añadido = true;
