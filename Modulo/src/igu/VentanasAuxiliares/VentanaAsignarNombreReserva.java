...
public class VentanaAsignarNombreReserva extends JDialog {
	...
	private JButton getBtnAceptar() {
		if (btnAceptar == null) {
			...
			btnAceptar
					.setToolTipText("Selecciona al pasajero para poner la reserva a su nombre");
			...
			btnAceptar.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (!txtNumeroContacto.getText().isEmpty()) {
						...
						((CardLayout) vp.getPnReservas().getLayout()).next(vp
								.getPnReservas());
						vp.crearResumen();
	...
	private JTextField getTxtNumeroContacto() {
		if (txtNumeroContacto == null) {
			...
			txtNumeroContacto
					.setToolTipText("Introduzca el n\u00FAmero de contacto para la reserva");
